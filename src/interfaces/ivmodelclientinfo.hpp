...
class IVModelInfo
{
	...
	virtual void GetModelMaterialColorAndLighting(const model_t *model, Vec3 const &origin, Vec3 const &angles,
						      trace_t *pTrace, Vec3 &lighting, Vec3 &matColor) = 0;
	virtual void GetIlluminationPoint(const model_t *model, IClientRenderable *pRenderable, Vec3 const &origin,
					  Vec3 const &angles, Vec3 *pLightingCenter)		   = 0;
	...
	virtual unsigned char ComputeLevelScreenFade(const Vec3 &vecAbsOrigin, float flRadius,
						     float flFadeScale) const				   = 0;
	virtual unsigned char ComputeViewScreenFade(const Vec3 &vecAbsOrigin, float flRadius,
						    float flFadeScale) const				   = 0;
	virtual int GetAutoplayList(const studiohdr_t *pStudioHdr, unsigned short **pAutoplayList) const   = 0;
	virtual CPhysCollide *GetCollideForVirtualTerrain(int index)					   = 0;
	virtual bool IsUsingFBTexture(const model_t *model, int nSkin, int nBody,
				      IClientRenderable* pClientRenderable) const		   = 0;
